...
Calentamiento previo a la charla (unos 5 minutos antes; NO toca el registro): `PYTHONPATH=src uv run python scripts/warmup.py` hace una pregunta de participante y una de Ponente con cuentas temporales, las borra y vacía los datos de ensayo. Solo vacía si no hay preguntas reales guardadas (`--no-reset` para no vaciar). Tarda unos 25 s calentando y hasta ~2 minutos con el vaciado.

Prueba de humo de punta a punta contra la API real (15 comprobaciones; se limpia sola y sirve de calentamiento; la de la documentación de AWS repite una vez porque depende de que el modelo use la herramienta): `uv run python scripts/smoke_test.py`. Tras `scripts/deploy_agent.sh`, haz commit de `infra/agent_image.auto.tfvars`: versiona la imagen desplegada.
