...
**Estado de los specs:** la visión, los requisitos y `docs/entity_model.md` ya cubren el dominio **"Pregúntale al repo"**: agente con RAG (Bedrock Knowledge Base + S3 Vectors), AgentCore Gateway, Memory de corto plazo, identidad con Cognito (perfiles Estudiante/Profesional/General y roles Asistente/Presentador) y Bedrock Guardrails. Los umbrales marcados `Needs review` (NFR-004, NFR-011, NFR-012) están por confirmar. Siguen pendientes el diagrama de casos de uso y las specs de los UCs, cada uno en su rama (`docs/charla/dominio-agente.md`).
