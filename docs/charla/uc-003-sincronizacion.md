...
- **Medir la recuperación con `retrieve` ahorra pruebas a ciegas (2026-09-24).** Consultar la Knowledge Base directamente (puntuación y ruta de cada fragmento) mostró por qué fallaban las respuestas, y confirmó la mejora tras quitar `tests/` y `scripts/`: para la primera pregunta de ejemplo (sobre la metodología), `docs/vision.md` pasó del puesto 6 a ocupar 2 de los 6 fragmentos junto con `CLAUDE.md`. Ver `pruebas-navegador.md`.
...
- **Un documento que cita la pregunta y su respuesta se convierte en la mejor fuente para esa pregunta (2026-09-24).** El informe de pruebas de navegador, que citaba la primera pregunta de ejemplo (sobre la metodología) con su respuesta, pasó a ser el primer fragmento recuperado (0,806 contra 0,717 de `docs/vision.md`) y el agente terminó narrando las pruebas al público. Ese informe y el checklist ya no se indexan (`WORK_LOGS` en `sync/rules.py`; UC-003 BR-001). Regla práctica: las bitácoras no van al índice y no se citan literalmente preguntas y respuestas en documentos que sí se indexan.
- **Cuando el repositorio no define algo con claridad, la recuperación no lo arregla:** para la primera pregunta de ejemplo (sobre la metodología) no había una definición nítida en ningún documento priorizado, y el agente mezclaba AIUP con la aplicación. La solución fue de contenido: `docs/faq.md`, curado, con las respuestas a los cinco botones de ejemplo y solo con hechos comprobables (cifras de `presupuesto.md`, la configuración real de guardrails y cuotas). Pasó a recuperarse primero (0,881) y las respuestas salieron correctas.
- **Probar un documento antes del merge:** como la sincronización lee de `main`, se puede subir el archivo al bucket con el metadato `git-sha` y lanzar una ingesta (`aws bedrock-agent start-ingestion-job`); la siguiente sincronización lo reconcilia con `main`. Sirve para medir sin abrir un PR por cada intento, pero el archivo tiene que acabar en `main`.
- **La variación del modelo existe:** en una misma pregunta (instalación) el agente escribió `uv install` en una muestra y `uv sync` en otras. Con el FAQ, 3 de 3 respuestas por la API fueron correctas; una muestra pequeña, no una garantía.
